  ...
  async (error) => {
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const response = await axios.post('http://127.0.0.1:8080/api/auth/token/refresh/', {
...
// const api = axios.create({
//   baseURL: process.env.REACT_APP_API_URL || 'http://127.0.0.1:8000/api', // Fallback for local dev
//   headers: {
//     'Content-Type': 'application/json',
//   },
// });
...
//         // Use the same baseURL for refresh token request
